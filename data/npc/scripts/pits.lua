...
function creatureSayCallback(cid, type, msg)
    ...
    if msgcontains(msg, 'permision') then
            selfSay('You want to Acquire Permissions for The Pits of Inferno Quest? If you want to say {hell}...', cid)
    ...
    elseif talkState[talkUser] == 1 then
        if msgcontains(msg, 'çopopo') then
            ...
            elseif msgcontains(msg, 'prices') then
                selfSay('A great Pits of Inferno cost 300000 golds.', cid)
                talkState[talkUser] = 0
            end
    ...
    elseif talkState[talkUser] == 2 then
        if msgcontains(msg, 'kiukiukiu') then
            ...
            elseif msgcontains(msg, 'hell') then
                selfSay('You really want to pay 300k in the Permissions Pits of Inferno Quest, Knowing You Back there dead?', cid)
            ...
            if msgcontains(msg, 'yes') then
                if getPlayerStorageValue(cid,50066) == -1 then
                    if(doPlayerRemoveMoney(cid, 1000) == TRUE) then
                        ...
                        selfSay('As you wish! You can pass the door now...', cid)
                    end
                else
        ...
        elseif talkState[talkUser] == 5 then
            if msgcontains(msg, 'yes') then
                if getPlayerStorageValue(cid,5079) == -1 then
                    ...
                    selfSay('As you wish! You can pass the door now and enter the door...', cid)
                end
            else
                selfSay('Then not. Get away and don\'t waste my time!', cid)
            end
